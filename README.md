...
- **Rate limit:** ~60 requests/min globally; the puller self-limits to 55/min. `page_size` is capped at 1000. `until` cannot be in the future.
...
| Mode | Columns |
|------|---------|
| default / `--campaign-regex` | `ppbno, variant, arm, arm_label, last_status, timestamp, last_status_timestamp` |
